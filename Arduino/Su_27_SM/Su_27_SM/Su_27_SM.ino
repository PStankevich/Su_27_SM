...
char incomingByte = 0;  // переменна€ дл€ хранени€ полученного байта
boolean flag = true;
int in;
String Data;
String DataNum;
...
void setup() {
	...
	//  //  lcd.init();                     // инициализаци€ LCD
	//  lcd.backlight();                // включаем подсветку
	//  lcd.clear();                    // очистка диспле€
	//  lcd.setCursor(2, 0);
...
void loop() {
	...
	if (Serial.available() > 0) {  //если есть доступные данные
		...
		if (incomingByte != '/') {
			...
			if (data[5] == 6) {  //≈сли это —у 25 _____________________________________________________________________________
				if (data[1] >= 0.24) {
			...
			else if (data[5] == 1) { //≈сли это —у 27 _________________________________________________________________________
				if (data[1] >= 1) {
					digitalWrite(flaps, HIGH);
				}
				else {
